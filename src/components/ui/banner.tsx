...
export const Banner = ({
  ...
  return (
    <BannerContext.Provider value={{ show, setShow }}>
      <div
        className={cn(
          'flex w-full items-center justify-between gap-2 px-4 py-2 border-blue-200 border-b bg-blue-50 dark:border-blue-900 dark:bg-blue-950',
          inset && 'rounded-lg',
          className
        )}
        {...props}
...
export const BannerIcon = ({
  ...
  <div
    className={cn(
      'rounded-full border border-background/20 bg-background/10 p-1 shadow-sm h-5 w-5 text-blue-600 dark:text-blue-400',
      className
    )}
    {...props}
  >
...
export const BannerTitle = ({ className, ...props }: BannerTitleProps) => (
  <p
    className={cn('flex-1 text-sm text-blue-700 dark:text-blue-300', className)}
    {...props}
  />
)
